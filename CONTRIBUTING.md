...
src/
  ...
  utils/           CSV/glob, file I/O, hashing, logging, pattern matching
...
5. Document the field in `README.md`'s Configuration Reference and the
   matching page under `docs/` (`docs/configuration.md`, `docs/labels.md`,
   etc.) if it's user-facing.
6. Add unit tests under `tests/unit/`.

## Updating the documentation site

`docs/` (built with [MkDocs](https://www.mkdocs.org/) + Material, config in
`mkdocs.yml`) is the source for the published site at
[rotsl.github.io/biotrace](https://rotsl.github.io/biotrace/). It's
user-facing only — no contributor/dev content belongs there; that stays in
this file and the root-level `.md` files.
...
To publish a change, run the **Deploy docs** workflow manually (Actions →
Deploy docs → Run workflow). It isn't triggered automatically on push.
...
- [ ] README (and `docs/`, if user-facing) updated if configuration, inputs, outputs, or labels changed
